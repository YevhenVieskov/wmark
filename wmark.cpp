...
// wmark.cpp : Определяет поведение классов для приложения.
//
...
BEGIN_MESSAGE_MAP(CwmarkApp, CWinApp)
...
// Единственный объект CwmarkApp

CwmarkApp theApp;


// инициализация CwmarkApp

BOOL CwmarkApp::InitInstance()
{
	...
	/*FILE*fp;
	fp = fopen("test", "w");
	fprintf(fp,"%s", dlg.m_esmsg);
	fclose(fp);*/
	//

	//Cstring csText;
	//std::string csText;
	//GetDlgItemText(dlg,IDC_EDIT_ESMSG, csText, 20);
